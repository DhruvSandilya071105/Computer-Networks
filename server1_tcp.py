...
def handle_client(conn, addr):
    print(f"Client {addr} connected to TCP Server 1")
    while True:
        try:
            message = conn.recv(1024).decode()
            if not message:
                break
            print(f"TCP Server 1 received: {message}")
...
def receive_from_server2():
    ...
    print("TCP Server 1 listening for Server 2 on port 9997...")
    while True:
        conn, addr = s2_listener.accept()
        message = conn.recv(1024).decode()
        print(f"TCP Server 1 received from Server 2: {message}")
...
server.listen(5)
print("TCP Server 1 running on port 9999...")
