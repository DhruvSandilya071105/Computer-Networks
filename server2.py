...
def handle_client(conn, addr):
    print(f"Client {addr} connected to Server 2")
    while True:
        try:
...
def receive_from_server1():
    ...
    print("Server 2 listening for Server 1 on port 9998...")
    while True:
        conn, addr = s1_listener.accept()
        message = conn.recv(1024).decode()
        print(f"Server 2 received from Server 1: {message}")
...
server.listen(5)
print("Server 2 running on port 8888...")
